...
export default function TopBar({ onSearch, searchQuery, accent, onNewEvent, onToggleDark, isDark, activeNav, onNavChange }) {
  ...
  const closeAll = () => { setShowNotif(false); setShowSettings(false); setShowMobileMenu(false); };

  const bg = isDark ? "rgba(15,23,42,0.97)" : "rgba(255,255,255,0.95)";
  const border = isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.07)";

  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-4 sm:px-6 gap-3"
        style={{ background: bg, backdropFilter: "blur(20px)", borderBottom: `1px solid ${border}`, boxShadow: "0 1px 12px rgba(0,0,0,0.06)" }}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <span className="font-black tracking-tight text-sm sm:text-base whitespace-nowrap"
            style={{ color: isDark ? "#f1f5f9" : "#1e293b" }}>
            <span className="hidden sm:inline">Interactive Calendar</span>
            <span className="sm:hidden">iCalendar</span>
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <div className="relative hidden sm:block">
            ...
            <input value={searchQuery} onChange={e => onSearch(e.target.value)}
              ...
              className="pl-9 pr-8 py-2 rounded-full text-sm outline-none w-36 lg:w-44 transition-all"
          ...
          <div className="relative">
            <button onClick={() => { setShowNotif(v => !v); setShowSettings(false); setShowMobileMenu(false); }}
            ...
            {showNotif && (
              <div className="slide-in absolute right-0 top-11 rounded-2xl overflow-hidden z-50"
                style={{ width: "min(320px, calc(100vw - 24px))", background: "#fff", boxShadow: "0 8px 32px rgba(0,0,0,0.14)", border: "1px solid #f1f5f9" }}>
                <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <span className="text-sm font-black text-slate-800">
                    Notifications {unread > 0 && <span className="text-xs font-bold text-red-500">({unread})</span>}
                  </span>
                  <div className="flex items-center gap-2">
                    {unread > 0 && <button onClick={markAll} className="text-xs font-semibold" style={{ color: accent, background: "none", border: "none", cursor: "pointer" }}>Mark all read</button>}
                    <button onClick={() => setShowNotif(false)} className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400 text-xs" style={{ background: "none", border: "none", cursor: "pointer" }}>✕</button>
                  </div>
                </div>
                {notifs.length === 0
                ...
                {notifs.length > 0 && (
                  <div className="px-4 py-2.5" style={{ borderTop: "1px solid #f1f5f9" }}>
                    <button onClick={clearAll} className="w-full text-xs font-bold py-2 rounded-xl hover:bg-slate-50"
                      style={{ color: "#ef4444", background: "none", border: "none", cursor: "pointer" }}>Clear all</button>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="relative hidden sm:block">
            <button onClick={() => { setShowSettings(v => !v); setShowNotif(false); }}
              ...
              <div className="slide-in absolute right-0 top-11 w-60 rounded-2xl overflow-hidden z-50"
                ...
                <div className="flex items-center justify-between px-4 py-3 hover:bg-slate-50">
                  <div className="flex items-center gap-3">
                    <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
                    <span className="text-sm font-medium text-slate-700">Dark Mode</span>
                  </div>
                  <button onClick={onToggleDark} className="press relative w-11 h-6 rounded-full flex-shrink-0"
                ...
                <button onClick={() => { setShowShortcuts(true); setShowSettings(false); }}
                  ...
                  <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 2l-4 5-4-5"/></svg>
                  <span className="text-sm font-medium text-slate-700">Keyboard Shortcuts</span>
                </button>
                <button onClick={() => { onNewEvent(); setShowSettings(false); }}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 text-left"
                  style={{ background: "none", border: "none", cursor: "pointer" }}>
                  <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
          ...
          {/* Avatar — desktop */}
          <div className="hidden sm:flex w-8 h-8 rounded-full items-center justify-center text-white text-xs font-black cursor-pointer select-none"
            style={{ background: accent, boxShadow: `0 2px 8px ${accent}55` }}>
            IC
          </div>

          {/* Hamburger — mobile only */}
          <button onClick={() => { setShowMobileMenu(v => !v); setShowNotif(false); setShowSettings(false); }}
            className="press md:hidden w-9 h-9 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors"
            style={{ background: "none", border: "none", cursor: "pointer" }} aria-label="Open menu">
            <svg className="w-5 h-5" style={{ color: "#64748b" }} fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              {showMobileMenu
      ...
      {/* Mobile drawer */}
      {showMobileMenu && (
        <div className="md:hidden fixed top-16 left-0 right-0 z-50 slide-in"
          style={{ background: isDark ? "#0f172a" : "#fff", borderBottom: "1px solid #e2e8f0", boxShadow: "0 8px 24px rgba(0,0,0,0.12)" }}>
          ...
          <div className="px-4 pt-4 pb-3">
            <div className="relative">
              ...
              />
              {searchQuery && (
                <button onClick={() => onSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-bold"
                  style={{ background: "none", border: "none", cursor: "pointer" }}>✕</button>
              )}
            </div>
          </div>
          ...
          <div className="px-4 pb-4" style={{ borderTop: "1px solid #f1f5f9" }}>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 px-2 pt-3">Settings</p>
            <div className="flex items-center justify-between px-4 py-3 rounded-xl mb-2"
              style={{ background: isDark ? "#1e293b" : "#f8fafc" }}>
              <div className="flex items-center gap-3">
                <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
                <span className="text-sm font-medium" style={{ color: isDark ? "#f1f5f9" : "#475569" }}>Dark Mode</span>
              </div>
              <button onClick={onToggleDark} className="press relative w-11 h-6 rounded-full"
                style={{ background: isDark ? accent : "#e2e8f0", border: "none", cursor: "pointer" }}>
                <span className="absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all"
                  style={{ left: isDark ? "calc(100% - 22px)" : "2px" }} />
              </button>
            </div>
            <button onClick={() => { onNewEvent(); setShowMobileMenu(false); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm" style={{ boxShadow: "0 24px 64px rgba(0,0,0,0.2)" }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowShortcuts(false)} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400"
                style={{ background: "none", border: "none", cursor: "pointer" }}>✕</button>
            </div>
            {[["←/→","Navigate months"],["T","Jump to today"],["N","New note"],["Y","Year view"],["Esc","Close"]].map(([k,d]) => (
              <div key={k} className="flex items-center justify-between py-2" style={{ borderBottom: "1px solid #f1f5f9" }}>
                <span className="text-sm text-slate-600">{d}</span>
                <kbd className="px-2.5 py-1 rounded-lg text-xs font-mono font-bold"
                  style={{ background: "#f1f5f9", color: accent, border: `1px solid ${accent}30` }}>{k}</kbd>
              </div>
            ))}
          </div>
        </div>
      ...
      {(showNotif || showSettings || showMobileMenu) && (
